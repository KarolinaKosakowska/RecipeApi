...
namespace RecipeApi.Auth
{
    ...
    {
        private IHttpContextAccessor contextAccessor;// mozemy czytac po request i response
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, KeyRequirement requirement)
        {
            ...
            if (key != null)
            {
                string role = roles.ContainsKey(key) ? roles[key] : PolicyEnum.Lack.ToString();//ContainsKey sprawdza czy istnieje taki klucz, jeli jest to go pobeieramy i przypisujemy jego wartość
